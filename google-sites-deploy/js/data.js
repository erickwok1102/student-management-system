...
class DataManager {
    ...
    // 實時同步點名記錄到Google Sheets
    async syncAttendanceToGoogleSheets(attendanceData) {
        if (!this.isGoogleSheetsEnabled) {
        ...
        try {
            // 檢查是否有點名工作表，如果沒有則創建
            await this.ensureAttendanceSheet();
            ...
            console.log('點名記錄已同步到Google Sheets');
            return true;
            
        } catch (error) {
            console.error('同步點名記錄到Google Sheets失敗:', error);
            throw error;
        }
    }

    // 確保點名工作表存在
    async ensureAttendanceSheet() {
        try {
            // 獲取工作表信息
            ...
            const attendanceSheet = sheets.find(sheet => sheet.properties.title === '點名記錄');
            ...
            if (!attendanceSheet) {
                ...
                await gapi.client.sheets.spreadsheets.values.update({
                    ...
                    valueInputOption: 'RAW',
                    resource: {
                        values: [['日期', '學員姓名', '班別', '狀態', '記錄時間']]
                    }
                });
                
                console.log('點名記錄工作表已創建');
            }
            
        } catch (error) {
            console.error('確保點名工作表存在失敗:', error);
            throw error;
        }
    }
